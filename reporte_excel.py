...
def pedidos(libro):
    ...
    reporte_pedidos.add_chart(grafico1, "N20")
    reporte_pedidos['A4'] = 'Week day'
    reporte_pedidos['A6'] = 'Month'
    reporte_pedidos['A4'].font = Font(bold=True, color='0000FF')
    reporte_pedidos['A6'].font = Font(bold=True, color='99004C')
    safe_book(libro)
...
def ingredientes(libro):
    ...
    reporte_ingredientes.add_chart(grafico1, "I59")
    safe_book(libro)
...
def ejecutivo(libro, revenues, pizzas_vendidas, cantidad_pizzas_pedido, revenues_per_week, pizzas_pedidas, a):
    ...
    sheet.pictures.add(fig, name='Revenues per week', update=True, left=sheet.range(
        'B5').left, top=sheet.range('H5').top)
    ####################################
    ...
    sheet.pictures.add(fig2, name='Most Sold Pizzas', update=True, left=sheet.range(
        'P7').left, top=sheet.range('H5').top)
